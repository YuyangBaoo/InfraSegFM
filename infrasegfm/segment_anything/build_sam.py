...
def build_sam_vit_h(image_size=256, keep_resolution=True, checkpoint=None):
    return _build_sam(
...
def build_sam_vit_l(image_size=256, keep_resolution=True, checkpoint=None):
    return _build_sam(
...
def build_sam_vit_b(image_size=256, keep_resolution=True, checkpoint=None):
    return _build_sam(
...
def _load_torch_weights(file_obj):
    try:
        return torch.load(file_obj, map_location="cpu", weights_only=True)
    except TypeError:
...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        if not checkpoint.exists():
            url = SAM_CHECKPOINT_URLS.get(checkpoint.name)
            msg = f"SAM checkpoint not found: {checkpoint}"
            if url:
                msg += f"\nDownload it from: {url}"
            raise FileNotFoundError(msg)
        with open(checkpoint, "rb") as f:
            state_dict = _load_torch_weights(f)
        try:
